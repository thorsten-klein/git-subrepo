...
def test_pull_theirs(env):
    ...
    if result.returncode != 0:
        ...
        subprocess.run(
            ['git', 'checkout', '--theirs', 'Bar2'], cwd=worktree_dir, check=True
        )
    ...
    assert_output_matches(
        bar2_content, expected, "The readme file in the mainrepo is theirs"
    )
    ...
    assert_output_matches(
        bar2_content, expected, "The readme file in the subrepo is theirs"
    )
